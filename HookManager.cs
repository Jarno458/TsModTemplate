...
//based on https://github.com/wledfor2/PlayHooky
namespace TsMod
{
	...
	public class HookManager
	{
		...
		byte[] PatchJmp(MethodInfo original, MethodInfo replacement)
		{
			//JIT compile methods
        ...
        static void UnhookJmp(MethodInfo original, byte[] originalOpcodes)
		{
			IntPtr originalSite = original.MethodHandle.GetFunctionPointer();
        ...
        static uint VirtualProtect(IntPtr address, uint size, uint protectionFlags)
		{
            //call VirtualProtect on the region of memory, 
			if (!Natives.VirtualProtect(address, (UIntPtr)size, protectionFlags, out var oldProtection))
			{
        ...
        static void FlushInstructionCache(IntPtr address, uint size)
		{
